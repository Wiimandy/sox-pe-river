...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $gh=$env:GH_EXE; $repo=$env:REPO; $branch=$env:BRANCH; $files=@('data.js','index.html','app.js','compile_data.py','plot_river.py','record_daily_tracking.py','splice_sox_yfinance_proxy.py','sox_pe_data_W.csv','sox_pe_river_data_W.csv','sox_daily_tracking_log.csv','sox_daily_tracking_log.xlsx'); foreach($file in $files){ if(Test-Path $file){ Write-Host \"Uploading $file\"; $sha=& $gh api \"repos/$repo/contents/$file\" --jq '.sha' 2>$null; $content=[Convert]::ToBase64String([IO.File]::ReadAllBytes($file)); $body=@{message='Auto-update SOX LSEG/yfinance proxy data'; content=$content; branch=$branch} ; if($sha){ $body.sha=$sha }; $body=$body | ConvertTo-Json -Compress; $body | & $gh api \"repos/$repo/contents/$file\" -X PUT --input - | Out-Null } }"
if %errorlevel% neq 0 (
    echo.
